...
class ChaiContainer(BaseContainer):
  ...
  def __init__(self, input_dir: str, output_dir: str, trunk_recycles: int,
               diffusion_steps: int, esm_embeddings: bool, max_parallel: int = 1):
    super().__init__(
  ...
  def run(self, client: docker.DockerClient = None):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_parallel) as executor:
      for i, chunks in enumerate(chunks):  # Cycling on all chunks
        device = f'cuda:{i % 2}'
